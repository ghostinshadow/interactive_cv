...
app.LayersControl = function(opt_options) {
  ...
  var control = ol.control.Control.call(this, {
    element: element,
...
app.LayersControl.prototype.setMap = function(map) {
  ...
  for (var i=0, ii=layers.length; i < ii; ++i) {
    ...
    if (title) {
      ...
      if(i > 0){
        var div_for_colorpicker = document.createElement("div");
        div_for_colorpicker.className = "colorpicker_map";
        div_for_colorpicker.id = layers_opts[i-1].name
        this.containers[group].appendChild(div_for_colorpicker);
        webix.ui({container: layers_opts[i-1].name, id: layers_opts[i-1].name, view:"colorpicker", width: 110, name:"color", value: '#0000ff' });
        $$(layers_opts[i-1].name).attachEvent("onChange", changeColorOfLayer);
      };
    }
  }
};

function changeColorOfLayer(newv, oldv) {
    var map = $('#map').data('map');
    var associate_level = map.getLayer(this.data.id);
    var source_layer = associate_level.getSource();
    ...
    source_layer.setStyle(new_style);
}